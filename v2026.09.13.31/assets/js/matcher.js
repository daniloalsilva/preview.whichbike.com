document.addEventListener("DOMContentLoaded", function () {
  ...
  function calculateMatches() {
    ...
    let scoredBikes = motorcycles
      .filter(bike => bike.spec_status !== "pending" && [
        bike.seat_height_mm, bike.wet_weight_kg, bike.power_hp
    ...
    // Keep the best-scoring year for each motorcycle model, then show a concise shortlist.
    const uniqueModels = scoredBikes.filter((item, index, items) => {
      const key = `${item.bike.brand}::${item.bike.model}`;
      return items.findIndex(candidate =>
        `${candidate.bike.brand}::${candidate.bike.model}` === key
      ) === index;
    });

    renderResults(uniqueModels.slice(0, MATCHER_RESULTS_LIMIT), uniqueModels.length);
  }

  function renderResults(scoredBikes, totalMatches) {
    const resultsList = document.getElementById("results-list");
    const resultsCount = document.getElementById("results-count");
    resultsList.innerHTML = "";
    const hiddenMatches = Math.max(0, totalMatches - scoredBikes.length);
    resultsCount.textContent = hiddenMatches > 0
      ? `Mostrando as ${scoredBikes.length} melhores opções entre ${totalMatches} modelos compatíveis.`
      : `${scoredBikes.length} ${scoredBikes.length === 1 ? 'modelo compatível encontrado' : 'modelos compatíveis encontrados'}.`;
    ...
    scoredBikes.forEach(item => {
      ...
      card.dataset.modelKey = `${bike.brand}::${bike.model}`;
